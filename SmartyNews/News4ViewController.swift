...
class News4ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource,UIWebViewDelegate,XMLParserDelegate {
    ...
    override func viewDidLoad() {
        ...
        //XMLを解析する(パース)
        let url:String = "https://news.yahoo.co.jp/pickup/entertainment/rss.xml"
        let urlToSend:URL = URL(string:url)!
        paser = XMLParser(contentsOf: urlToSend)!
        totalBox = []
        paser.delegate = self
        paser.parse()
    ...
    //タグの間にデータがあったとき(開始タグと終了タグでくくられた箇所にデータが存在したとき実行されるメソッド)
